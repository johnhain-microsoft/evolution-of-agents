...
try {
    az rest --method POST `
        ...
        --body "{`"principalId`":`"$principalId`",`"resourceId`":`"$graphSpId`",`"appRoleId`":`"$calendarsReadId`"}" `
...
try {
    az rest --method POST `
        --uri "https://graph.microsoft.com/v1.0/servicePrincipals/$graphSpId/appRoleAssignedTo" `
        --body "{`"principalId`":`"$principalId`",`"resourceId`":`"$graphSpId`",`"appRoleId`":`"$calendarsReadWriteId`"}" `
        --headers "Content-Type=application/json" | Out-Null
